...
class TrackDayControllerTest {
    ...
    void testGetTrackDayByDate() throws Exception {
        ...
        when(service.getRepository().findByDay(any(Date.class))).thenReturn(trackDay);
    ...
    void testGetTrackDaysStatisticByMonthAndYear() throws Exception {
        ...
        mockMvc.perform(MockMvcRequestBuilders.get("/api/track-days/statistic/{month}/{year}", 9, 2024)
                .contentType(MediaType.APPLICATION_JSON))
                .andExpect(MockMvcResultMatchers.status().isOk())
                .andExpect(MockMvcResultMatchers.jsonPath("$[0].temperature").value(statisticDto.getTemperature()))
                .andDo(print());
    }

    @Test
    void testStoreTrackDay() throws Exception {
        TrackDayDto trackDayDto = new TrackDayDto(36.5f, new Date(), null, null, null, null, null, null, null, null,
                null, null);
        ...
        mockMvc.perform(MockMvcRequestBuilders.post("/api/track-days")
                ...
                .andExpect(MockMvcResultMatchers.header().string("Location", "http://localhost/api/track-days/1"))
                .andDo(print());
    }
